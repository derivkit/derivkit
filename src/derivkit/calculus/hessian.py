...
def build_hessian_tensor(function: Callable,
                  theta0: np.ndarray,
                  n_workers: int=1
) -> NDArray[np.floating]:
    """Returns the hessian of a vector-valued function.
    ...
    Returns:
        A 3D array representing the hessian.
    ...
    if f0.ndim != 1:
        raise TypeError(
            f"build_hessian_tensor expects f: R^n -> R^m with 1-D vector output; got shape {f0.shape}"
        )
...
def _hessian_component_tensor(function: Callable, theta0: np.ndarray, i: int, j:int, n_workers: int) -> float:
    """Return one entry of the Hessian for a scalar-valued function.
