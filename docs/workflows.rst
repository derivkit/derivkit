...
different inference regimes. Fisher bias addresses a complementary question:
how parameter inference shifts when the data-generating model differs from
the model assumed in analysis. ``DerivKit`` does not guarantee the validity
of any approximation for a given problem; assessing whether a method is
appropriate for a specific model, parameterization, prior choice, and
scientific goal requires scientific judgment from the user.
...
- a baseline (inference) model mapping parameters to observables
- a more complete data-generating model that includes an additional systematic
- a fiducial parameter vector ``theta0`` and a data covariance
- (optionally) an existing Fisher matrix

**We compute**

- the difference vector
  :math:`\Delta \nu = \nu_{\rm complete}(\theta_0) - \nu_{\rm baseline}(\theta_0)`
- the induced first-order parameter shift :math:`\Delta\theta` under the baseline model

**Use**

- :meth:`ForecastKit.fisher_bias` (and :meth:`ForecastKit.delta_nu` to build ``delta_nu``)
...
- In practice, ``delta_nu`` is constructed by evaluating *two model predictions*
  at the **same** fiducial parameters: a baseline model used for inference and a
  more complete model that includes the systematic.
- The resulting ``delta_theta`` estimates how best-fit parameters would shift if
  data generated by the more complete model were analyzed using the baseline model
  alone (i.e. the systematic is *neglected*).
- For the conceptual overview and interpretation, see :ref:`forecastkit-fisher-bias`.
...
**When should I worry about Fisher bias?**

When your inference model is an approximation to the true data-generating
process. If neglected physical or instrumental systematics produce a
non-negligible shift in the predicted observables, Fisher bias estimates the
resulting first-order parameter shift under the assumed model. See
:ref:`forecastkit-fisher-bias` for interpretation and
